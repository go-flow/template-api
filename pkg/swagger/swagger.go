...
func WrapHandler(h *webdav.Handler, confs ...func(c *Config)) flow.HandlerFunc {
...
func CustomWrapHandler(config *Config, h *webdav.Handler) flow.HandlerFunc {
	...
	return func(c *flow.Context) {
		...
		switch path {
		case "index.html":
			index.Execute(c.Response, &swaggerUIBundle{
				URL:         config.URL,
		...
		case "doc.json":
			...
			return
		case ".ambassador-internal/openapi-docs":
			doc, err := swag.ReadDoc()
			if err != nil {
				panic(err)
			}
...
func DisablingWrapHandler(h *webdav.Handler, envName string) flow.HandlerFunc {
...
// if specified environment variable passed
func DisablingCustomWrapHandler(config *Config, h *webdav.Handler, envName string) flow.HandlerFunc {
	eFlag := os.Getenv(envName)
	if eFlag != "" {
		return func(c *flow.Context) {
			// Simulate behavior when route unspecified and
